...
class BankApp:
    ...
    def add_loan(self, loan_type: str):
        if loan_type == "MortgageLoan" or loan_type == "StudentLoan":
    ...
    def grant_loan(self, loan_type: str, client_id: str):
        client = [c for c in self.clients if c.client_id == client_id][0]  # вадим си клиента
        # търсим дали е студент / adult  и дали заема съответства
    ...
    def get_statistics(self):
        total_clients_income = sum([i.income for i in self.clients])
        ...
        try:
            avg_client_interest_rate = sum([i.interest for i in self.clients]) / len(self.clients)
        except ZeroDivisionError:
            avg_client_interest_rate = 0

        results = f"Active Clients: {len(self.clients)}\n"
        results += f"Total Income: {total_clients_income:.2f}\n"
        results += f"Granted Loans: {loans_count_granted_to_clients}, Total Sum: {granted_sum:.2f}\n"
        results += f"Available Loans: {len(self.loans)}, Total Sum: {not_granted_sum:.2f}\n"
        results += f"Average Client Interest Rate: {avg_client_interest_rate:.2f}"
